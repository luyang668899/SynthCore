...
import { WeChatPlugin } from "./index.js";
...
async function testWeChatPlugin() {
  console.log("Testing WeChat Plugin...");

  // Create mock runtime
  const runtime = new MockRuntime();

  // Create plugin instance
  const plugin = new WeChatPlugin(runtime);
  ...
  try {
    ...
    // Wait for WeChat connection to complete
    console.log("\nWaiting for WeChat connection to complete...");
    ...
    // Test 8: Get chats
    console.log("\n8. Testing getChats...");
    const chats = await plugin.listChannels();
    console.log("✓ Chats retrieved:", chats);

    // Test 9: Get contacts
    console.log("\n9. Testing getContacts...");
    const contacts = await plugin.listUsers();
    ...
    const mediaMessage = await plugin.sendMedia("1234567890", "https://example.com/image.jpg", {
      caption: "Test image",
    });
    ...
    const channel = await plugin.getChannel("1234567890");
    console.log("✓ Channel retrieved:", channel);

    // Test 14: Get user
    console.log("\n14. Testing getUser...");
    const user = await plugin.getUser("1234567890");
    ...
    await plugin.onReady(() => console.log("WeChat ready"));
    await plugin.onDisconnect((reason: any) => console.log("WeChat disconnected:", reason));
    ...
    console.log("\n🎉 All tests passed! WeChat Plugin is working correctly.");
  } catch (error) {
