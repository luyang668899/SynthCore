...
import { SlackEnhancedPlugin } from "./index";
...
async function testSlackEnhancedPlugin() {
  console.log("Testing Slack Enhanced Plugin...");

  // Create mock runtime
  const runtime = new MockRuntime();

  // Create plugin instance
  const plugin = new SlackEnhancedPlugin(runtime);
  ...
  try {
    ...
    const channel = await plugin.getChannel("C12345");
    console.log("✓ Channel retrieved:", channel);

    // Test 11: Get user
    console.log("\n11. Testing getUser...");
    const user = await plugin.getUser("U12345");
    console.log("✓ User retrieved:", user);

    // Test 12: Send message
    console.log("\n12. Testing sendMessage...");
    ...
    // Test 13: Send message with attachments
    console.log("\n13. Testing sendMessageWithAttachments...");
    const messageWithAttachments = await plugin.sendMessageWithAttachments(
      "C12345",
    ...
    console.log("✓ Message with attachments sent successfully:", messageWithAttachments);

    // Test 14: Send message with blocks
    console.log("\n14. Testing sendMessageWithBlocks...");
    const messageWithBlocks = await plugin.sendMessageWithBlocks("C12345", [
      {
        type: "section",
        text: {
          type: "mrkdwn",
          text: "*Hello* from OpenClaw with blocks!",
        },
      },
    ]);
    console.log("✓ Message with blocks sent successfully:", messageWithBlocks);

    // Test 15: Join channel
    console.log("\n15. Testing joinChannel...");
    const joined = await plugin.joinChannel("C12345");
    console.log("✓ Channel joined successfully:", joined);

    // Test 16: Leave channel
    console.log("\n16. Testing leaveChannel...");
    const left = await plugin.leaveChannel("C12345");
    ...
    console.log("\n🎉 All tests passed! Slack Enhanced Plugin is working correctly.");
  } catch (error) {
    console.error("❌ Test failed:", error);
  }
}

testSlackEnhancedPlugin();
