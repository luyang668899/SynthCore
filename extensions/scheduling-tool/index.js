...
const scheduling_manager_1 = require("./src/scheduling-manager");
function register() {
  const schedulingManager = new scheduling_manager_1.SchedulingManager();
  return {
    commands: {
      ...
      "enable-job": async (args) => {
        try {
          ...
          return {
            success: false,
            message: `Failed to enable job: ${error.message}`,
          };
        }
      },
      "disable-job": async (args) => {
        try {
          ...
          return {
            success: false,
            message: `Failed to disable job: ${error.message}`,
          };
        }
      },
      "run-job": async (args) => {
