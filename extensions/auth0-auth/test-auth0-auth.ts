import { Auth0AuthPlugin } from "./index";
...
async function testAuth0AuthPlugin() {
  console.log("=== Testing Auth0 Auth Plugin ===\n");

  // Create mock API
  const api = new MockOpenClawPluginApi();

  // Register plugin
  await Auth0AuthPlugin.register(api as any);
  console.log("\n");

  try {
    // Test 1: Get auth URL
    console.log("Test 1: Get Auth0 Auth URL");
    const authUrlResult = await api.callTool("auth0_get_auth_url", { state: "test-state-123" });
    console.log("Auth URL:", authUrlResult.authUrl);
    console.log("✓ Auth URL generated successfully");
    console.log("\n");

    // Test 2: Handle callback
    console.log("Test 2: Handle Auth0 Callback");
    const callbackResult = await api.callTool("auth0_handle_callback", { code: "test-code-123" });
    ...
    // Test 3: Verify token
    console.log("Test 3: Verify Auth0 Token");
    const verifyResult = await api.callTool("auth0_verify_token", { token });
    console.log("Verified user:", verifyResult.user);
    console.log("✓ Token verified successfully");
    console.log("\n");

    // Test 4: Get session status
    console.log("Test 4: Get Auth0 Session Status");
    const sessionResult = await api.callTool("auth0_session_status", { userId });
    ...
    console.log("Test 5: Update Auth0 Configuration");
    const configResult = await api.callTool("auth0_config", {
      auth0: {
    ...
    // Test 6: Logout
    console.log("Test 6: Auth0 Logout");
    const logoutResult = await api.callTool("auth0_logout", { userId });
    ...
    const sessionAfterLogoutResult = await api.callTool("auth0_session_status", { userId });
    ...
    console.log("=== All Auth0 Auth Plugin Tests Passed! ===");
  } catch (error) {
    console.error("Test failed:", error);
  } finally {
    // Unregister plugin
    await Auth0AuthPlugin.unregister(api as any);
  }
}
