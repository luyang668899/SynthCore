...
import { LinePlugin } from "./index.js";
...
async function testLinePlugin() {
  console.log("Testing Line Plugin...");

  // Create mock runtime
  const runtime = new MockRuntime();

  // Create plugin instance
  const plugin = new LinePlugin(runtime);
  ...
  try {
    ...
    // Wait for Line connection to complete
    console.log("\nWaiting for Line connection to complete...");
    ...
    // Test 9: Send media
    console.log("\n9. Testing sendMedia...");
    const mediaMessage = await plugin.sendMedia("U1234567890", "https://example.com/image.jpg", {
      caption: "Test image",
    });
    ...
    const channel = await plugin.getChannel("U1234567890");
    console.log("✓ Channel retrieved:", channel);

    // Test 11: Get user
    console.log("\n11. Testing getUser...");
    const user = await plugin.getUser("U1234567890");
    ...
    const joined = await plugin.joinChannel("C1234567890");
    console.log("✓ Channel join result:", joined);

    // Test 15: Leave channel
    console.log("\n15. Testing leaveChannel...");
    const left = await plugin.leaveChannel("C1234567890");
    ...
    await plugin.onReady(() => console.log("Line ready"));
    await plugin.onDisconnect((reason: any) => console.log("Line disconnected:", reason));
    ...
    console.log("\n🎉 All tests passed! Line Plugin is working correctly.");
  } catch (error) {
