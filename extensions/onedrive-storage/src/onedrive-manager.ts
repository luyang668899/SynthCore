...
export interface OneDriveFile {
  ...
  permissions: {
    ...
    type: "user" | "group" | "organization" | "anyone";
    email?: string;
  }[];
}
...
export class OneDriveManager {
  private files: Map<string, OneDriveFile> = new Map();
  ...
  private initializeMockData() {
    ...
    this.files.set("file-1", {
      ...
      shared: false,
      webUrl: "https://onedrive.live.com/view.aspx?resid=FILE123",
      permissions: [
        {
          role: "owner",
          type: "user",
    ...
    this.files.set("file-2", {
      ...
      shared: true,
      webUrl: "https://onedrive.live.com/view.aspx?resid=FILE456",
      permissions: [
        {
          role: "owner",
          type: "user",
    ...
    this.files.set("folder-2", {
      ...
      shared: false,
      webUrl: "https://onedrive.live.com/?cid=FOLDER456",
      permissions: [
        {
          role: "owner",
          type: "user",
  ...
  // 文件操作方法
  async uploadFile(options: UploadOptions): Promise<OneDriveFile> {
    if (!this.accessToken) {
    ...
    const newFile: OneDriveFile = {
      ...
      webUrl: `https://onedrive.live.com/view.aspx?resid=${fileId}`,
  ...
  async updateFilePermissions(
    ...
    file.shared = permissions.some((p) => p.type === "anyone" || p.type === "organization");
  ...
  // 文件夹操作方法
  async createFolder(path: string): Promise<OneDriveFile> {
    if (!this.accessToken) {
      throw new Error("Not authenticated");
    }
    ...
    const newFolder: OneDriveFile = {
      ...
      lastModified: new Date().toISOString(),
      shared: false,
      webUrl: `https://onedrive.live.com/?cid=${folderId}`,
      permissions: [
        {
          role: "owner",
          type: "user",
  ...
  async listFolder(folderId: string = "root"): Promise<OneDriveFile[]> {
    if (!this.accessToken) {
      throw new Error("Not authenticated");
    }
  ...
  // 搜索方法
  async search(options: SearchOptions): Promise<OneDriveFile[]> {
    if (!this.accessToken) {
      throw new Error("Not authenticated");
    }
