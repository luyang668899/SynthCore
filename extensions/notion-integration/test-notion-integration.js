// Simple test script for Notion integration plugin
const { NotionManager } = require("./dist/notion-manager");

console.log("Testing Notion integration plugin...");

// Test 1: Check if the NotionManager class is properly defined
console.log("\n1. Testing NotionManager class definition...");
try {
  const config = {
    apiKey: "mock-api-key",
  };
  const notionManager = new NotionManager(config);
  console.log("✅ NotionManager class is properly defined");
  console.log("✅ NotionManager instance created successfully");

  // Test 2: Check if all required methods exist
  console.log("\n2. Testing NotionManager methods...");
  ...
  for (const method of requiredMethods) {
    if (typeof notionManager[method] === "function") {
  ...
  console.log("\nNote: API functionality tests require valid Notion credentials.");
